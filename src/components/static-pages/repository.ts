...
class Repository {
  ...
  ): Promise<ReadonlyArray<StaticPage>> {
  ...
  public static async create(data: StaticPage): Promise<StaticPage> {
  ...
  public static async update({ id, ...data }: any): Promise<StaticPage> {
  ...
  public static async delete(id: string): Promise<StaticPage> {
    const document = await Model.findById(id).exec();
  ...
  public static async generatedDedicatedData(objects: ReadonlyArray<StaticPage>): Promise<void> {
...
const transform = (document: Document): StaticPage => {
