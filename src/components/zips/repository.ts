...
class Repository {
  ...
  public static async create(data: Zip): Promise<Zip> {
  ...
  public static async update({ id, ...data }: any): Promise<Zip> {
  ...
  public static async delete(id: string): Promise<Zip> {
    const document = await Model.findById(id).exec();
...
const transform = (document: Document): Zip => {
